    ...
    return(
        <div className="flex justify-around">
            {selectedDiseases.map((disease, index) => (
                <div ref={el => boxRefs.current[index] = el} key={disease.name} className="flex flex-col items-center justify-center py-3 px-5 bg-red-200 rounded-lg shadow-custom01 animation-fade-in">
                    <div className="image-container w-12 h-12 overflow-hidden">
                        <img src={disease.icon || images.no_img} alt={disease.name} className="w-full h-full object-contain" />
                    </div>
